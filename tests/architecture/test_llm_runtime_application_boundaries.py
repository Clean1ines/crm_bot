...
LLM_RUNTIME_APPLICATION = ROOT / "src" / "contexts" / "llm_runtime" / "application"
...
def test_llm_runtime_application_does_not_encode_cross_context_semantics() -> None:
    offenders: list[str] = []
    compiled_patterns = [
        re.compile(pattern) for pattern in FORBIDDEN_CROSS_CONTEXT_PATTERNS
    ]

    for path in LLM_RUNTIME_APPLICATION.rglob("*.py"):
    ...
    assert not offenders, (
        "LLM Runtime application policies must stay provider-neutral and business-agnostic. "
        "They must not encode Workbench, execution, artifact, source, transport, or "
        "business semantics:\n" + "\n".join(offenders)
    )
