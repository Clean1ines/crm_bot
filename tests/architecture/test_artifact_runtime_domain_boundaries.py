...
ARTIFACT_RUNTIME_DOMAIN = ROOT / "src" / "contexts" / "artifact_runtime" / "domain"
...
def test_artifact_runtime_domain_does_not_encode_cross_context_semantics() -> None:
    offenders: list[str] = []
    compiled_patterns = [
        re.compile(pattern) for pattern in FORBIDDEN_CROSS_CONTEXT_PATTERNS
    ]

    for path in ARTIFACT_RUNTIME_DOMAIN.rglob("*.py"):
    ...
    assert not offenders, (
        "Artifact Runtime domain must stay generic. It must not encode Workbench, "
        "execution, LLM, provider, source, transport, or business semantics:\n"
        + "\n".join(offenders)
    )
