...
def test_workbench_observability_uses_real_canonical_fact_schema() -> None:
    source = OBSERVABILITY_REPOSITORY.read_text(encoding="utf-8")

    for marker in FORBIDDEN_SCHEMA_DRIFT:
        assert marker not in source


def test_evidence_trace_surfaces_uses_real_canonical_fact_columns() -> None:
    source = OBSERVABILITY_REPOSITORY.read_text(encoding="utf-8")
    method = _method(source, "list_evidence_trace_surfaces", "list_workbench_documents")

    assert "f.claim AS title" in method
    assert "f.claim AS claim" in method
    assert "COALESCE(f.possible_questions, '[]'::jsonb)" in method
    assert "COALESCE(f.claim_kind, 'other')" in method
    assert "ON m.fact_id = f.fact_id" in method
    assert "AND m.registry_id = f.registry_id" in method


def test_import_quality_surfaces_uses_real_fact_mentions_join() -> None:
    source = OBSERVABILITY_REPOSITORY.read_text(encoding="utf-8")
    method = _method(
        source,
