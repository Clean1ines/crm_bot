...
KNOWLEDGE_PREPROCESSING_DOMAIN = (
...
def test_stage_k_ingestion_does_not_expose_or_use_legacy_chunk_projection() -> None:
    source = _source(INGESTION_SERVICE)
    domain_source = _source(KNOWLEDGE_PREPROCESSING_DOMAIN)

    assert "result.to_chunks()" not in source
    assert "def to_chunk(" not in domain_source
    assert "def to_chunks(" not in domain_source
...
def test_stage_k_preprocessor_port_has_no_legacy_known_answer_path() -> None:
    ...
    assert "previous_question_intents" not in source
