...
def _work_item_attempt(
    error_kind: LlmErrorKind = LlmErrorKind.REQUEST_TOO_LARGE,
) -> WorkItemAttempt:
...
def _split_artifact(
    ...
    return PipelineArtifact(
        artifact_ref=ArtifactRef("split-artifact-1"),
...
def _command(
    *,
    ...
    return RecordClaimExtractionSplitRequiredCommand(
        ...
        split_artifact=_split_artifact(),
        occurred_at=_now(),
    )
...
def test_record_claim_extraction_split_required_supersedes_parent_work_item() -> None:
    ...
    assert result.superseded_work_item.status is WorkItemStatus.SPLIT_SUPERSEDED
    assert result.superseded_work_item.leased_by is None
    assert result.superseded_work_item.lease_token is None
    assert result.superseded_work_item.lease_expires_at is None

    assert isinstance(result.work_item_event, WorkItemSplitSuperseded)
    assert isinstance(result.llm_event, LlmTaskFailed)
    assert result.llm_event.error_kind is LlmErrorKind.REQUEST_TOO_LARGE
    assert isinstance(result.split_artifact_event, ArtifactStored)

    assert unit_of_work.saved_work_items == [result.superseded_work_item]
    ...
    assert unit_of_work.saved_artifacts == [_split_artifact()]
    assert unit_of_work.appended_events == [
        result.work_item_event,
        result.llm_event,
        result.split_artifact_event,
    ]
    assert unit_of_work.committed
    assert not unit_of_work.rolled_back


def test_record_claim_extraction_split_required_accepts_output_too_large() -> None:
    unit_of_work = FakeClaimExtractionWorkItemUnitOfWork()
    ...
    assert result.llm_event.error_kind is LlmErrorKind.OUTPUT_TOO_LARGE
    assert result.superseded_work_item.status is WorkItemStatus.SPLIT_SUPERSEDED


def test_record_claim_extraction_split_required_rolls_back_when_commit_fails() -> None:
...
def test_record_claim_extraction_split_required_rejects_non_split_error_kind() -> None:
    with pytest.raises(ValueError):
        _command(
            llm_task=_llm_task(error_kind=LlmErrorKind.VALIDATION_FAILED),
        )


def test_record_claim_extraction_split_required_rejects_non_failed_llm_task() -> None:
    with pytest.raises(ValueError):
        _command(
            llm_task=_llm_task(status=LlmTaskStatus.SUCCEEDED),
        )


def test_record_claim_extraction_split_required_requires_timezone_aware_occurred_at() -> (
    None
):
    with pytest.raises(ValueError):
        RecordClaimExtractionSplitRequiredCommand(
