...
SAGA_SOURCE = ROOT / "src" / "contexts" / "knowledge_workbench" / "application" / "sagas" / "knowledge_extraction_saga.py"
...
def _state(checkpoints: tuple[KnowledgeExtractionPhaseCheckpoint, ...] = ()) -> KnowledgeExtractionWorkflowState:
...
def _unit(unit_ref: str, ordinal: int) -> SourceUnit:
    return SourceUnit(SourceUnitRef(unit_ref), SourceDocumentRef("source-document-1"), SourceUnitKind.SECTION, SourceUnitText("section"), HeadingPath(("Section",)), SourceUnitLineage(), ordinal, _now())
...
class _StateRepository(KnowledgeExtractionSagaStateRepositoryPort):
    ...
    async def load_workflow_state(self, workflow_run_id: str) -> KnowledgeExtractionWorkflowState | None:
        return self.state

    async def save_workflow_state(self, state: KnowledgeExtractionWorkflowState) -> None:
        self.saved_states.append(state)

    async def save_phase_checkpoint(self, checkpoint: KnowledgeExtractionPhaseCheckpoint) -> None:
...
class _EventCursor(KnowledgeExtractionEventCursorPort):
    ...
    async def record_processed_event(self, record: KnowledgeExtractionEventCursorRecord) -> None:
...
class _CommandEmitter(KnowledgeExtractionCommandEmitterPort):
    ...
    async def emit_command(self, *, command_key: str, target_context: str, command_kind: str, payload: Mapping[str, object]) -> None:
        self.commands.append({"command_key": command_key, "target_context": target_context, "command_kind": command_kind, "payload": dict(payload)})
...
def _saga(state_repository: _StateRepository, source_repository: _SourceRepository | None = None) -> tuple[KnowledgeExtractionSaga, _CommandLog, _EventCursor, _CommandEmitter]:
    command_log = _CommandLog()
    event_cursor = _EventCursor()
    command_emitter = _CommandEmitter()
    reconciler = None if source_repository is None else KnowledgeExtractionSourcePhaseReconciler(source_repository=source_repository)
    saga = KnowledgeExtractionSaga(state_repository=state_repository, command_log=command_log, event_cursor=event_cursor, command_emitter=command_emitter, source_phase_reconciler=reconciler)
...
async def test_existing_behavior_is_unchanged_without_source_reconciler() -> None:
    ...
    result = await saga.reconcile(ReconcileKnowledgeExtractionSagaCommand("workflow-1", _now()))
...
async def test_missing_source_document_saves_blocked_checkpoint_and_pauses() -> None:
    ...
    saga, command_log, event_cursor, command_emitter = _saga(state_repository, source_repository)

    result = await saga.reconcile(ReconcileKnowledgeExtractionSagaCommand("workflow-1", _now()))

    assert [checkpoint.phase_key for checkpoint in state_repository.saved_checkpoints] == [KnowledgeExtractionPhaseKey.SOURCE_DOCUMENT_PERSISTED, KnowledgeExtractionPhaseKey.SOURCE_UNITS_CREATED]
    assert state_repository.saved_checkpoints[0].phase_status is KnowledgeExtractionPhaseStatus.BLOCKED
    assert state_repository.saved_checkpoints[1].phase_status is KnowledgeExtractionPhaseStatus.NOT_STARTED
    assert len(state_repository.saved_states) == 1
    assert state_repository.saved_states[0].status is KnowledgeExtractionWorkflowStatus.PAUSED
    assert state_repository.saved_states[0].current_phase is KnowledgeExtractionPhaseKey.SOURCE_DOCUMENT_PERSISTED
...
@pytest.mark.asyncio
async def test_source_document_present_but_units_missing_pauses_at_source_units() -> None:
    state_repository = _StateRepository(_state())
    saga, _, _, _ = _saga(state_repository, _SourceRepository(_document()))

    result = await saga.reconcile(ReconcileKnowledgeExtractionSagaCommand("workflow-1", _now()))

    assert state_repository.saved_checkpoints[0].phase_status is KnowledgeExtractionPhaseStatus.COMPLETED
    assert state_repository.saved_checkpoints[1].phase_status is KnowledgeExtractionPhaseStatus.NOT_STARTED
    assert state_repository.saved_states[0].status is KnowledgeExtractionWorkflowStatus.PAUSED
    assert state_repository.saved_states[0].current_phase is KnowledgeExtractionPhaseKey.SOURCE_UNITS_CREATED
...
async def test_source_document_and_units_present_completes_source_checkpoints() -> None:
    ...
    assert state_repository.saved_checkpoints[0].phase_status is KnowledgeExtractionPhaseStatus.COMPLETED
    assert state_repository.saved_checkpoints[1].phase_status is KnowledgeExtractionPhaseStatus.COMPLETED
    assert state_repository.saved_checkpoints[1].completed_count == 2
    assert state_repository.saved_states[0].status is KnowledgeExtractionWorkflowStatus.RUNNING
    assert state_repository.saved_states[0].current_phase is KnowledgeExtractionPhaseKey.SOURCE_UNITS_CREATED
...
async def test_replaces_existing_source_checkpoints_in_saved_state() -> None:
    old_checkpoint = KnowledgeExtractionPhaseCheckpoint("workflow-1", KnowledgeExtractionPhaseKey.SOURCE_DOCUMENT_PERSISTED, KnowledgeExtractionPhaseStatus.NOT_STARTED)
    state_repository = _StateRepository(_state((old_checkpoint,)))
    saga, _, _, _ = _saga(state_repository, _SourceRepository(_document(), (_unit("source-document-1.unit.0", 0),)))

    await saga.reconcile(ReconcileKnowledgeExtractionSagaCommand("workflow-1", _now()))

    phase_keys = [checkpoint.phase_key for checkpoint in state_repository.saved_states[0].checkpoints]
...
def test_source_guard() -> None:
    ...
    required_markers = ("KnowledgeExtractionSourcePhaseReconciler", "_source_phase_reconciler", "save_phase_checkpoint", "KnowledgeExtractionPhaseCheckpoint", "SOURCE_DOCUMENT_PERSISTED", "SOURCE_UNITS_CREATED", "source_document_missing", "source_units_missing")
    forbidden_markers = ("emit_command(", "record_command(", "event_was_processed(", "record_processed_event(", "PostgresSourceManagementRepository", "PostgresKnowledgeExtractionSagaStateRepository", "asyncpg", "postgres", "Postgres", "src.infrastructure", "JobDispatcher", "worker_loop", "outbox_events", "published_at", "Groq", "Qwen", "execution_runtime", "llm_runtime", "artifact_runtime", "knowledge_workbench_documents", "knowledge_workbench_document_sections", "knowledge_workbench_processing_runs", "SectionBatchQueueItem", "workbench_parallel_processing", "process_workbench_document", "RunClaimExtractionStageAsync", "RecordClaimExtractionSuccess", "ProcessClaimExtractionWorkItem", "ApplyDraftClaimObservationArtifactAsync", "PROMPT_A_WORK_SCHEDULED")
