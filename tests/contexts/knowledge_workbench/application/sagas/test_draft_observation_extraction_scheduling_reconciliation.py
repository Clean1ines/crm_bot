...
RECONCILER = ROOT / "src" / "contexts" / "knowledge_workbench" / "application" / "sagas" / "knowledge_extraction_draft_observation_scheduling_reconciliation.py"
...
def _source_units_checkpoint(status: KnowledgeExtractionPhaseStatus, count: object) -> KnowledgeExtractionPhaseCheckpoint:
...
@pytest.mark.asyncio
async def test_source_units_checkpoint_missing_returns_not_ready_without_index_call() -> None:
    work_index = _WorkIndex(0)
    decision = await DraftObservationExtractionSchedulingReconciler(work_index=work_index).reconcile_scheduling(_state())

    assert decision.expected_source_unit_count == 0
    assert decision.scheduled_work_item_count == 0
    assert decision.status is DraftObservationExtractionSchedulingStatus.SOURCE_UNITS_NOT_READY
    assert decision.suggested_checkpoint_status() is KnowledgeExtractionPhaseStatus.NOT_STARTED
    assert work_index.calls == []


@pytest.mark.asyncio
async def test_source_units_checkpoint_not_completed_returns_not_ready_without_index_call() -> None:
    work_index = _WorkIndex(0)
    checkpoint = _source_units_checkpoint(KnowledgeExtractionPhaseStatus.NOT_STARTED, 2)

    decision = await DraftObservationExtractionSchedulingReconciler(work_index=work_index).reconcile_scheduling(_state((checkpoint,)))
...
@pytest.mark.asyncio
async def test_source_units_completed_with_zero_count_returns_not_ready_without_index_call() -> None:
    work_index = _WorkIndex(0)
...
async def test_ready_to_schedule_when_no_work_items_exist() -> None:
    ...
    assert decision.scheduled_work_item_count == 0
    assert decision.status is DraftObservationExtractionSchedulingStatus.READY_TO_SCHEDULE
    assert decision.suggested_checkpoint_status() is KnowledgeExtractionPhaseStatus.READY
...
async def test_partially_scheduled_when_some_work_items_exist() -> None:
    ...
    decision = await DraftObservationExtractionSchedulingReconciler(work_index=_WorkIndex(1)).reconcile_scheduling(_state((checkpoint,)))

    assert decision.status is DraftObservationExtractionSchedulingStatus.PARTIALLY_SCHEDULED
    assert decision.suggested_checkpoint_status() is KnowledgeExtractionPhaseStatus.IN_PROGRESS
...
async def test_already_scheduled_when_enough_or_more_work_items_exist() -> None:
    ...
    exact = await DraftObservationExtractionSchedulingReconciler(work_index=_WorkIndex(2)).reconcile_scheduling(_state((checkpoint,)))
    extra = await DraftObservationExtractionSchedulingReconciler(work_index=_WorkIndex(3)).reconcile_scheduling(_state((checkpoint,)))

    assert exact.status is DraftObservationExtractionSchedulingStatus.ALREADY_SCHEDULED
    assert exact.suggested_checkpoint_status() is KnowledgeExtractionPhaseStatus.COMPLETED
...
async def test_invalid_checkpoint_payload_raises() -> None:
    ...
    bad_payload = _source_units_checkpoint(KnowledgeExtractionPhaseStatus.COMPLETED, "2")
    reconciler = DraftObservationExtractionSchedulingReconciler(work_index=_WorkIndex(0))
...
def test_decision_validation_rejects_bad_inputs() -> None:
    with pytest.raises(ValueError):
        DraftObservationExtractionSchedulingDecision(" ", "source-document-1", 0, 0, DraftObservationExtractionSchedulingStatus.SOURCE_UNITS_NOT_READY)
    with pytest.raises(ValueError):
        DraftObservationExtractionSchedulingDecision("workflow-1", " ", 0, 0, DraftObservationExtractionSchedulingStatus.SOURCE_UNITS_NOT_READY)
    with pytest.raises(ValueError):
        DraftObservationExtractionSchedulingDecision("workflow-1", "source-document-1", -1, 0, DraftObservationExtractionSchedulingStatus.SOURCE_UNITS_NOT_READY)
    with pytest.raises(ValueError):
        DraftObservationExtractionSchedulingDecision("workflow-1", "source-document-1", 1, -1, DraftObservationExtractionSchedulingStatus.READY_TO_SCHEDULE)
