...
@pytest.mark.asyncio
async def test_persist_artifact_commits_stored_artifact_and_event() -> None:
    unit_of_work = FakeArtifactUnitOfWork()

    result = await PersistArtifact(unit_of_work=unit_of_work).execute(_command())
...
@pytest.mark.asyncio
async def test_persist_artifact_preserves_lineage() -> None:
    unit_of_work = FakeArtifactUnitOfWork()
    ...
    result = await PersistArtifact(unit_of_work=unit_of_work).execute(command)
...
@pytest.mark.asyncio
async def test_persist_artifact_rolls_back_when_commit_fails() -> None:
    unit_of_work = FakeArtifactUnitOfWork(fail_on_commit=True)

    with pytest.raises(RuntimeError, match="commit failed"):
        await PersistArtifact(unit_of_work=unit_of_work).execute(_command())
