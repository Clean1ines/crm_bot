...
from src.contexts.artifact_runtime.application.use_cases.list_artifacts_for_resume import (
    ListArtifactsForResume,
    ListArtifactsForResumeCommand,
)
...
def test_list_artifacts_for_resume_delegates_to_parent_ref_lookup() -> None:
    parent_ref = ArtifactRef("parent-artifact")
    matching = _artifact(
        artifact_ref="matching-artifact",
        parent_refs=(parent_ref,),
    )
    unrelated = _artifact(artifact_ref="unrelated-artifact")
    repository = FakeArtifactRepository((matching, unrelated))
    use_case = ListArtifactsForResume(repository=repository)

    result = use_case.execute(ListArtifactsForResumeCommand(parent_ref=parent_ref))

    assert result.artifacts == (matching,)
    assert repository.list_by_parent_ref_calls == [parent_ref]


def test_list_artifacts_for_resume_does_not_mutate_repository() -> None:
    parent_ref = ArtifactRef("parent-artifact")
    matching = _artifact(
        artifact_ref="matching-artifact",
        parent_refs=(parent_ref,),
    ...
    use_case.execute(ListArtifactsForResumeCommand(parent_ref=parent_ref))
