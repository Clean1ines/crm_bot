...
def test_groq_free_plan_seed_contains_target_text_models_in_fallback_order() -> None:
    ...
    assert [profile.model_rank for profile in profiles] == [0, 1, 2, 3, 4]
...
def test_gpt_oss_seeds_represent_reasoning_controls_without_disable_none() -> None:
    profiles = build_groq_free_plan_model_profiles()
    gpt_oss_20b = profiles[2]
    gpt_oss_120b = profiles[3]

    for profile in (gpt_oss_20b, gpt_oss_120b):
        assert profile.reasoning_profile.supports_reasoning_control
        assert not profile.reasoning_profile.can_disable_reasoning
        assert profile.reasoning_profile.default_effort is ReasoningEffort.MEDIUM
        assert profile.max_output_tokens == 65_536
        assert profile.rate_limits.requests_per_day == 1_000
...
def test_llama_70b_seed_uses_lower_daily_token_limit_and_32768_output() -> None:
    llama_70b = build_groq_free_plan_model_profiles()[4]
