...
async def _raise_invalid_prompt_a_output(
    *,
...
async def _raise_non_prompt_a_domain_error(
    *,
    payload: WorkbenchParallelProcessingJobPayloadDto | Mapping[str, object],
    connection: object,
) -> object:
...
@pytest.mark.asyncio
async def test_invalid_prompt_a_output_is_retryable_and_does_not_mark_document_failed(
    ...
    monkeypatch.setattr(
        ...
        _raise_invalid_prompt_a_output,
    )
...
@pytest.mark.asyncio
async def test_non_prompt_a_domain_error_still_marks_document_failed(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    connection = FakeConnection()
    monkeypatch.setattr(
        terminal,
        "handle_workbench_parallel_processing_job_from_connection",
        _raise_non_prompt_a_domain_error,
    )
