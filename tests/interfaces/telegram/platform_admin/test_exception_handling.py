...
async def test_add_manager_failure_returns_safe_message_without_raw_exception():
    with (
        ...
        patch(
            "src.interfaces.telegram.platform_admin.handlers.PlatformBotService"
        ) as Service,
        patch("src.interfaces.telegram.platform_admin.handlers.logger") as logger,
    ):
        service = MagicMock()
        service.add_manager_by_chat_id = AsyncMock(
...
async def test_knowledge_upload_failure_returns_safe_fallback_and_logs_context():
    with (
        ...
        patch(
            "src.interfaces.telegram.platform_admin.knowledge_upload._get_file_path",
        ...
        patch(
            "src.interfaces.telegram.platform_admin.knowledge_upload._get_project_menu_keyboard",
            AsyncMock(return_value=None),
        ),
        patch(
            "src.interfaces.telegram.platform_admin.knowledge_upload.logger"
        ) as logger,
    ):
