...
def test_surface_publish_endpoint_has_required_fail_fast_and_status_transitions() -> None:
    ...
    assert 'raise HTTPException(status_code=400, detail="Surface does not belong to document")' in source
    assert 'raise HTTPException(status_code=409, detail="Surface has no linked runtime entry yet")' in source
