...
class TestWebhooks:
    def test_explicit_client_webhook_success(self, client):
        ...
        with patch(
            "src.interfaces.http.webhooks.process_client_update", new_callable=AsyncMock
        ) as process:
    ...
    def test_explicit_platform_webhook_success(self, client):
        ...
        with patch(
            "src.interfaces.http.webhooks.settings.ADMIN_BOT_TOKEN", admin_token
        ):
            with patch(
                "src.interfaces.http.webhooks.settings.PLATFORM_WEBHOOK_SECRET",
                secret_token,
            ):
                with patch(
                    "src.interfaces.http.webhooks.process_admin_update",
        ...
        assert response.json() == {"ok": True}
        process.assert_awaited_once_with(
            expected_update, app.dependency_overrides[get_pool]()
        )
    ...
    def test_explicit_manager_webhook_success(self, client):
        ...
        members = AsyncMock()
        members.get_manager_notification_targets = AsyncMock(
            return_value=[str(manager_chat_id)]
        )
        members.resolve_manager_user_id_by_telegram = AsyncMock(
            return_value="11111111-1111-1111-1111-111111111111"
        )
        app.dependency_overrides[get_project_member_repo] = lambda: members

        with patch(
            "src.interfaces.http.webhooks.process_manager_update",
            new_callable=AsyncMock,
        ) as process:
            process.return_value = {"ok": True}
            response = client.post(
    ...
    def test_explicit_manager_webhook_success_via_membership_notification_target(
        self, client
    ):
    ...
    def test_webhook_client_success(self, client):
        ...
        with patch(
            ...
            with patch(
                "src.interfaces.http.webhooks.process_client_update",
                new_callable=AsyncMock,
            ) as process:
                process.return_value = {"ok": True}
                response = client.post(
    ...
    def test_manager_webhook_success(self, client):
        ...
        tokens = AsyncMock()
        tokens.find_project_by_manager_webhook_secret = AsyncMock(
...
def test_legacy_webhook_unexpected_error_uses_safe_500_contract():
    ...
    tokens = AsyncMock()
    tokens.get_bot_token = AsyncMock(
