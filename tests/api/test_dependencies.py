...
class TestVerifyAdminToken:
    ...
    async def test_success(self):
        with patch("src.api.dependencies.settings") as mock_settings:
            mock_settings.ADMIN_API_TOKEN = "admin-token"
            await verify_admin_token(authorization="Bearer admin-token")
    ...
    async def test_invalid_format(self):
        with pytest.raises(HTTPException) as exc:
            await verify_admin_token(authorization="Invalid")
        assert exc.value.status_code == 401
        assert exc.value.detail == "Invalid token format. Use 'Bearer <token>'"
    ...
    async def test_wrong_token(self):
        with patch("src.api.dependencies.settings") as mock_settings:
            mock_settings.ADMIN_API_TOKEN = "correct"
            with pytest.raises(HTTPException) as exc:
...
class TestPoolAndOrchestrator:
    def test_get_pool_success(self):
        with patch("src.api.dependencies.src.core.lifespan.pool", "mock_pool"):
            result = get_pool()
            assert result == "mock_pool"

    def test_get_pool_not_initialized(self):
        with patch("src.api.dependencies.src.core.lifespan.pool", None):
            with pytest.raises(RuntimeError, match="Database pool not initialized"):
                get_pool()

    def test_get_orchestrator_success(self):
        with patch("src.api.dependencies.src.core.lifespan.orchestrator", "mock_orch"):
            result = get_orchestrator()
            assert result == "mock_orch"

    def test_get_orchestrator_not_initialized(self):
        with patch("src.api.dependencies.src.core.lifespan.orchestrator", None):
...
class TestRepositoryFactories:
    def test_get_project_repo(self):
        mock_pool = MagicMock()
        with patch("src.api.dependencies.ProjectRepository") as MockRepo:
            get_project_repo(pool=mock_pool)
            MockRepo.assert_called_once_with(mock_pool)

    def test_get_thread_repo(self):
        mock_pool = MagicMock()
        with patch("src.api.dependencies.ThreadRepository") as MockRepo:
            get_thread_repo(pool=mock_pool)
            MockRepo.assert_called_once_with(mock_pool)

    def test_get_memory_repository(self):
        mock_pool = MagicMock()
        with patch("src.api.dependencies.MemoryRepository") as MockRepo:
...
class TestGetRedis:
    ...
    async def test_get_redis(self):
        ...
        with patch("src.api.dependencies.get_redis_client", return_value=mock_client) as mock_get:
