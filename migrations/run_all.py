...
async def apply_migrations_for_env(env_file):
    ...
    if is_prod_env(env_file):
        ...
        response = (
            input("Продолжить? (введите 'yes' для подтверждения): ").strip().lower()
        )
        if response != "yes":
    ...
    try:
        ...
        for filepath in sql_files:
            ...
            try:
                ...
                print(
                    f"{Colors.RED}❌ Error executing {filepath.name}: {e}{Colors.RESET}"
                )
                raise

        print(
            f"{Colors.GREEN}🎉 Миграции для {env_file.name} успешно применены{Colors.RESET}"
        )

    except Exception as e:
        print(
            f"{Colors.RED}❌ Ошибка при применении миграций для {env_file.name}: {e}{Colors.RESET}"
        )
    finally:
        if conn:
            await conn.close()
...
async def main():
    ...
    if not env_files:
        ...
        print(
            "   Создайте нужные файлы или добавьте имена в ALLOWED_ENV_FILES в скрипте."
        )
        sys.exit(1)
