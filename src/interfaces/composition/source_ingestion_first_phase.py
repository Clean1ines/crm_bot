...
class SourceIngestionFirstPhaseSegmentationConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.planned_output_tokens, int):
            raise TypeError("planned_output_tokens must be int")
        if self.planned_output_tokens < 0:
            raise ValueError("planned_output_tokens must be >= 0")
        if (
            self.prompt_token_count + self.planned_output_tokens
            >= self.max_request_input_tokens
        ):
            raise ValueError(
                "prompt_token_count + planned_output_tokens must be "
...
def segmentation_config_from_profile(
    ...
    return SourceIngestionFirstPhaseSegmentationConfig(
        ...
        planned_output_tokens=profile.primary_model.planned_output_tokens,
    )
    ...
    async def execute(
        ...
        try:
            ...
            frontend_event_repository = CollectingFrontendWorkflowEventRepository(
                PostgresFrontendWorkflowEventRepository(connection),
            )
