        ...
        try:
            async with connection.transaction():
                ...
                LOGGER.info(
                    ...
                    input_tokens=(preparation_profile.input_tokens),
                    artifact_tokens=(preparation_profile.artifact_tokens),
                    required_window_tokens=preparation_profile.required_window_tokens,
                    estimated_tpm_input_tokens=(preparation_profile.input_tokens),
                    request_count=preparation_profile.request_count,
    ...
    def request_output_cap_decision(
        ...
        *,
        ...
        return request_output_cap_policy.decide(
            ...
            artifact_tokens=minimum_output_tokens,
    ...
    def can_admit(
        ...
        *,
        ...
        return (
            ...
            and decision.max_completion_tokens is not None
            and decision.max_completion_tokens >= minimum_output_tokens
            and self.remaining_minute_tokens >= decision.required_window_tokens
            and self.remaining_daily_tokens >= decision.required_window_tokens
        )
...
async def _lease_input_admitted_work_items(
    *,
    ...
    candidates = _input_admitted_candidates(
        ...
        model_max_output_tokens=capacity_limits.output_token_limit,
...
def _pop_first_record_that_fits(
    *,
    ...
    for index, record in enumerate(records):
        ...
        minimum_output_tokens = _artifact_tokens_from_due_record(record)
        ...
        account.consume(required_window_tokens=decision.required_window_tokens)
        records.pop(index)
        return _InputAdmittedCandidate(
            ...
            required_window_tokens=decision.required_window_tokens,
        )
    return None
...
def _input_tokens_from_due_record(record: DueWorkItemRecord) -> int:
    ...
    if isinstance(value, bool) or not isinstance(value, int):
        raise TypeError("llm_capacity_estimate.input_tokens must be int")
    if value <= 0:
        raise ValueError("llm_capacity_estimate.input_tokens must be > 0")
    return value


def _artifact_tokens_from_due_record(record: DueWorkItemRecord) -> int:
    estimate_payload = record.schedule_payload.get("llm_capacity_estimate")
    if not isinstance(estimate_payload, Mapping):
        raise ValueError("schedule_payload.llm_capacity_estimate is required")

    value = estimate_payload.get("artifact_tokens")
    if isinstance(value, bool) or not isinstance(value, int):
        raise TypeError("llm_capacity_estimate.artifact_tokens must be int")
    if value < 0:
        raise ValueError("llm_capacity_estimate.artifact_tokens must be >= 0")
    return value
...
def _admitted_schedule_payload(
    *,
    ...
    updated_estimate["max_completion_tokens"] = max_completion_tokens
    if max_completion_tokens is not None:
        updated_estimate["max_completion_tokens"] = max_completion_tokens
    else:
        updated_estimate.pop("max_completion_tokens", None)
    updated_estimate["required_window_tokens"] = required_window_tokens
...
def _capacity_reservation_from_started_attempt(
    *,
    ...
    return LlmRouteCapacityReservation(
        ...
        reserved_requests=1,
        reserved_tokens=_mapping_positive_int(estimate, "required_window_tokens"),
    ...
    *,
    ...
    blocked_accounts = tuple(
        ...
        and (
            ...
            or capacity.remaining_minute_tokens < profile.required_window_tokens
        )
    )
