...
def make_project_dynamic_keyboard(project_id: str, has_client_bot: bool, has_manager_bot: bool) -> InlineKeyboardMarkup:
    buttons = []
    ...
    buttons.append([InlineKeyboardButton("Удалить проект", callback_data=f"delete:{project_id}")])
    buttons.append([InlineKeyboardButton("Назад", callback_data="back_to_main")])
    logger.debug(
        "Built dynamic keyboard",
        extra={"project_id": project_id, "has_client_bot": has_client_bot, "has_manager_bot": has_manager_bot},
    )
    return InlineKeyboardMarkup(buttons)
