...
class ApplyDraftClaimCompactionCapacityAdmissionPhasePlanCommand:
...
@dataclass(frozen=True, slots=True)
class ApplyDraftClaimCompactionCapacityAdmissionPhasePlanResult:
    workflow_run_id: str
...
@dataclass(frozen=True, slots=True)
class DraftClaimCompactionCapacityAdmissionPhasePlanApplier:
    async def execute(
        self,
        command: ApplyDraftClaimCompactionCapacityAdmissionPhasePlanCommand,
        *,
        workflow_unit_of_work: WorkflowRuntimeUnitOfWorkPort,
    ) -> ApplyDraftClaimCompactionCapacityAdmissionPhasePlanResult:
        ...
        return ApplyDraftClaimCompactionCapacityAdmissionPhasePlanResult(
...
def _validate_mapping_plan(mapping_plan: CapacityAdmissionPhaseMappingPlan) -> None:
    if mapping_plan.profile != DRAFT_CLAIM_COMPACTION_ADMISSION_PHASE_PROFILE:
        raise ValueError(
            "mapping_plan must use draft-claim-compaction admission profile"
        )
...
def _workflow_event_payload(
    *,
    ...
    payload: dict[str, object] = {
        ...
        "dispatch_contexts": _dispatch_context_payloads(mapping_plan),
    }
    ...
    if (
        ...
        payload["reason"] = "primary_model_daily_capacity_exhausted"
    return payload
...
def _execute_command_from_summary(
    *,
    ...
    idempotency_key = (
        "execute-draft-claim-compaction:"
...
def _execute_command_payload(
    *,
    ...
    if dispatch_context is None:
        raise ValueError("draft claim compaction execute command requires context")
    _copy_dispatch_context(payload, dispatch_context)
    return payload


def _dispatch_context_payloads(
    mapping_plan: CapacityAdmissionPhaseMappingPlan,
) -> tuple[dict[str, object], ...]:
    contexts: list[dict[str, object]] = []
    for admitted_item in mapping_plan.admission_result.admitted_items:
        if admitted_item.dispatch_context is None:
            continue
        context: dict[str, object] = {
            "work_item_id": admitted_item.work_item_id,
        }
        _copy_dispatch_context(context, admitted_item.dispatch_context)
...
def _copy_dispatch_context(
    ...
    payload["group_ref"] = _required_text(dispatch_context.group_ref, "group_ref")
    payload["batch_ref"] = _required_text(dispatch_context.batch_ref, "batch_ref")
    payload["round_index"] = (
        dispatch_context.round_index if dispatch_context.round_index is not None else 0
    )
    payload["expected_output_kind"] = _required_text(
        dispatch_context.expected_output_kind,
        "expected_output_kind",
    )
    if dispatch_context.input_node_refs:
        payload["source_node_refs"] = dispatch_context.input_node_refs
    if dispatch_context.input_claim_refs:
        payload["source_claim_refs"] = dispatch_context.input_claim_refs
...
async def _save_progress_snapshot(
    *,
    ...
    domain_counters["draft_claim_compaction_user_model_choice_required_count"] = (
        1
    ...
    await workflow_unit_of_work.progress_snapshots.save_snapshot(
        WorkflowProgressSnapshot(
            ...
            current_phase=DRAFT_CLAIM_COMPACTION_ADMISSION_PHASE_PROFILE.phase,
    ...
    *,
    ...
    return WorkflowTimelineEntry(
        timeline_entry_id=(
            ...
            "DraftClaimCompactionCapacityAdmissionPhasePlanApplied:"
            f"{occurred_at.isoformat()}"
        ),
        workflow_run_id=workflow_command.workflow_run_id,
        event_type="DraftClaimCompactionCapacityAdmissionPhasePlanApplied",
        phase=DRAFT_CLAIM_COMPACTION_ADMISSION_PHASE_PROFILE.phase,
        severity=WorkflowTimelineSeverity.INFO,
        message="Draft claim compaction capacity admission phase plan applied",
        payload_summary={
            "workflow_run_id": workflow_command.workflow_run_id,
            "decision": mapping_plan.decision.value,
            "prepared_dispatch_count": mapping_plan.prepared_dispatch_count,
        ...
        occurred_at=occurred_at,
        source_ref=DRAFT_CLAIM_COMPACTION_ADMISSION_PHASE_PROFILE.work_kind,
    )


def _required_text(value: str | None, field_name: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise ValueError(f"dispatch context must include {field_name}")
    return value
