...
from src.contexts.knowledge_workbench.application.sagas.knowledge_extraction_saga_state import KnowledgeExtractionPhaseStatus, KnowledgeExtractionWorkflowState
...
class SourcePhaseReconciliationResult:
    ...
    def __post_init__(self) -> None:
        ...
        if self.document_phase_status is not _document_status(self.source_document_present):
    ...
    def suggested_checkpoint_status_for_document(self) -> KnowledgeExtractionPhaseStatus:
        if self.document_phase_status is SourcePhaseReconciliationStatus.SOURCE_DOCUMENT_PRESENT:
            return KnowledgeExtractionPhaseStatus.COMPLETED
        return KnowledgeExtractionPhaseStatus.BLOCKED

    def suggested_checkpoint_status_for_source_units(self) -> KnowledgeExtractionPhaseStatus:
        if self.source_units_phase_status is SourcePhaseReconciliationStatus.SOURCE_UNITS_PRESENT:
            return KnowledgeExtractionPhaseStatus.COMPLETED
...
class KnowledgeExtractionSourcePhaseReconciler:
    ...
    async def reconcile_source_phases(self, state: KnowledgeExtractionWorkflowState) -> SourcePhaseReconciliationResult:
        ...
        if document is None:
            return SourcePhaseReconciliationResult(state.workflow_run_id, state.source_document_ref, False, 0, SourcePhaseReconciliationStatus.SOURCE_DOCUMENT_MISSING, SourcePhaseReconciliationStatus.SOURCE_UNITS_MISSING)
        units = await self._source_repository.list_source_units_for_document(document_ref)
        unit_count = len(units)
        return SourcePhaseReconciliationResult(state.workflow_run_id, state.source_document_ref, True, unit_count, SourcePhaseReconciliationStatus.SOURCE_DOCUMENT_PRESENT, _units_status(unit_count))
