...
class FaqClaimObsContractGenerator(FaqWorkbenchClaimObservationsGenerator):
    def _parse_claim_observation(
        self, raw_observation: dict[str, JsonValue], *, index: int
    ) -> dict[str, JsonValue]:
        normalized = dict(raw_observation)
        normalized["scope"] = (
            self._optional_str(raw_observation, "scope", index=index) or "__global__"
        )
        parsed = dict(super()._parse_claim_observation(normalized, index=index))
        parsed["scope"] = (
            self._optional_str(raw_observation, "scope", index=index) or ""
        )
    ...
    def _relations(
        self, payload: dict[str, JsonValue], *, index: int
    ) -> list[JsonValue]:
        ...
        for pos, item in enumerate(raw):
            ...
            out.append(
                {
                    "target_ref": self._required_str(
                        item, "target_ref", index=index, label=f"relation {pos}"
                    ),
                    "relation": self._controlled_str(
                        item,
                        "relation",
                        allowed=_ALLOWED,
                        index=index,
                        label=f"relation {pos}",
                    ),
                    "reason": self._required_str(
                        item, "reason", index=index, label=f"relation {pos}"
                    ),
                }
            )
        return out
