...
async def _handle_notify_manager(
    ...
    target_manager_chat_id = (
        payload.get("target_manager_telegram_chat_id")
        or payload.get("manager_chat_id")
    )
    ...
    client_id = thread_info.client_id
    ...
    if (target_manager_user_id or target_manager_chat_id) and not manager_targets:
        logger.warning(
            "Target manager could not be resolved for notification",
            extra={
                "project_id": project_id,
                "manager_user_id": target_manager_user_id,
                "manager_chat_id": target_manager_chat_id,
                "job_id": job["id"],
            },
        )
    ...
    for target in manager_targets:
        mgr_chat_id = target.telegram_chat_id
