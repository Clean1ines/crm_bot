    ...
    async def get_project_settings(
        ...
        async with self.pool.acquire() as conn:
            ...
            project_settings["bot_token"] = self._decrypt_if_present(
                project_settings["bot_token"]
            )
            project_settings["manager_bot_token"] = self._decrypt_if_present(
                project_settings["manager_bot_token"]
            )
        ...
        targets = [
            str(
                r.get("manager_chat_id")
                if r.get("manager_chat_id") is not None
                else r.get("telegram_id")
            )
