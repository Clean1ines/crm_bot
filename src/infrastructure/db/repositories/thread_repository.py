...
class ThreadRepository:
    ...
    async def claim_for_manager(
        ...
        *,
        ...
        """
        Mark a thread as manually handled by a platform user.

        manager_chat_id is kept only as a Telegram transport bridge while the
        canonical project role and assignment use manager_user_id.
        """
    ...
    async def release_manager_assignment(self, thread_id: str) -> None:
        """
        Return a manual thread to AI handling and clear manager assignment.
        """
    ...
    async def get_thread_with_project_view(self, thread_id: str) -> ThreadWithProjectView | None:
        """Return a typed thread/project read model for application and runtime layers."""
        logger.debug(f"Fetching thread with project for thread {thread_id}")
        async with self.pool.acquire() as conn:
            row = await conn.fetchrow("""
                SELECT 
                    t.id, t.client_id, t.status,
                    t.manager_user_id, t.manager_chat_id,
                    t.context_summary, t.created_at, t.updated_at,
                    c.project_id, c.full_name, c.username, c.chat_id
                FROM threads t
    ...
    async def append_manager_reply_message(self, thread_id: str, content: str) -> None:
        """
        Atomically persist a manager reply as an assistant-visible message.
        """
    ...
    async def update_analytics(
        ...
        Args:
            ...
            intent: Detected intent, for example "pricing" or "support".
            lifecycle: Стадия жизненного цикла (например, "cold", "warm", "hot").
            cta: Тип призыва к действию (например, "call_manager").
            decision: Router decision, for example "RESPOND_KB".
        """
    ...
    async def get_message_counts_view(self, thread_id: str) -> ThreadMessageCounts:
        """Return typed message-count totals for a thread."""
