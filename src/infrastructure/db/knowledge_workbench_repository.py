...
class KnowledgeWorkbenchRepository(
    ...
    async def create_section_batch_plan(
        ...
        await self._connection.execute(
            ...
            ON CONFLICT (batch_plan_id) DO NOTHING
            """,
            plan.batch_plan_id,
            plan.processing_run_id,
            plan.project_id,
            ...
            plan.base_snapshot_sequence_number,
            plan.max_concurrency,
            plan.status.value,
            self._time(plan.created_at),
            self._time(plan.updated_at),
        )
    ...
    async def create_section_work_items(
        ...
        for item in items:
            await self._connection.execute(
                ...
                item.based_on_snapshot_id,
                item.based_on_snapshot_sequence_number,
                item.findings_node_run_id,
                item.dedup_node_run_id,
                item.registry_application_node_run_id,
                item.parsed_artifact_id,
                item.normalized_artifact_id,
                item.applied_snapshot_id,
                item.locked_by,
                self._time(item.locked_until),
                item.retry_count,
                item.dirty_reason,
                self._time(item.created_at),
                self._time(item.updated_at),
    ...
    async def update_section_work_items(
        ...
        for item in items:
            await self._connection.execute(
                ...
                UPDATE knowledge_workbench_section_batch_queue_items
                SET status = $2,
                    based_on_snapshot_id = $3,
                    based_on_snapshot_sequence_number = $4,
                    findings_node_run_id = $5,
                    dedup_node_run_id = $6,
                    registry_application_node_run_id = $7,
                    parsed_artifact_id = $8,
                    normalized_artifact_id = $9,
                    applied_snapshot_id = $10,
                    locked_by = $11,
                    locked_until = $12,
                    retry_count = $13,
                    dirty_reason = $14,
        ...
        *,
        ...
        row = await self._connection.fetchrow(
            ...
            SELECT
                ...
                created_at,
                updated_at
            FROM knowledge_workbench_parallel_section_batch_plans
            WHERE project_id = $1
              AND document_id = $2
              AND processing_run_id = $3
            ORDER BY created_at DESC NULLS LAST, batch_plan_id DESC
            LIMIT 1
            """,
            project_id,
            document_id,
        ...
        *,
        ...
        return tuple(
            WorkbenchSectionWorkItem(
                ...
                findings_node_run_id=str(row["findings_node_run_id"])
                if row["findings_node_run_id"] is not None
                else None,
                dedup_node_run_id=str(row["dedup_node_run_id"])
                if row["dedup_node_run_id"] is not None
                else None,
                registry_application_node_run_id=str(
                    row["registry_application_node_run_id"]
                )
                if row["registry_application_node_run_id"] is not None
                else None,
                parsed_artifact_id=str(row["parsed_artifact_id"])
                if row["parsed_artifact_id"] is not None
                else None,
                normalized_artifact_id=str(row["normalized_artifact_id"])
                if row["normalized_artifact_id"] is not None
                else None,
                applied_snapshot_id=str(row["applied_snapshot_id"])
                if row["applied_snapshot_id"] is not None
                else None,
