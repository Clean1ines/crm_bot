...
- config: {{KB_THRESHOLD:0.6, LLM_THRESHOLD:0.70}}
...
5) Если none of the above → LLM_GENERATE: сгенерируй вежливый, полезный ответ. 
...
8) Если пользователь задал несколько вопросов в одном сообщении (например, разделенных точкой, вопросительным знаком, переносом строки или списком), ответь на каждый из них по порядку. Объедини ответы в один текст, разделяя их пустой строкой или нумерацией. Убедись, что ответ покрывает все вопросы. Для этого используй decision = "LLM_GENERATE" и сформируй составной ответ в поле response.
9) При формировании JSON-ответа убедись, что он строго валиден: все строки в двойных кавычках, специальные символы (например, кавычки внутри текста) экранированы обратным слешем. Не используй неэкранированные управляющие символы.
...
def create_router_node(llm: Optional[ChatGroq] = None):
    ...
    async def router_node(state: AgentState) -> Dict[str, Any]:
        ...
        # We'll keep as is, but graph will map using conditional edges.
        result = {
