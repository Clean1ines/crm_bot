...
| work planning | `plan_claim_builder_section_work.py` | Add RAG Eval planners: one qgen work item per published runtime entry, one adjudication work item per failed/ambiguous generated question. |
...
| execution validator | `ClaimBuilderLlmDispatchOutputValidator` | Add qgen validator requiring exactly 10 questions; add adjudication validator requiring canonical classification. |
| outcome persistence | `draft_claim_observation_persistence` | Persist generated questions and adjudication results/candidates via RAG Eval repository. |
| progress reconcile | `handle_reconcile_claim_builder_progress_command.py` | Add phase-specific reconcile for now/later/drained/blocked and transition to retrieval, adjudication, promotion, verification. |
...
- promotion candidate grouping handler;
...
- Frontend `frontend/src/pages/rag-eval/RagEvalPage.tsx` must render phases: scope, qgen schedule/prepare/execute/reconcile, retrieval, adjudication, promotion, verification, accept/rollback.
...
- only adjudicated `VALID_TARGET_QUERY` questions become candidates;
...
3. Checkpoint C: retrieval outcome model/schema, retrieval evaluation handler, adjudication work kind/planner/prepare/execute/reconcile, promotion candidate filtering after `VALID_TARGET_QUERY`.
4. Checkpoint D: grouped promotion application with reversible embedding revisions, post-promotion verification, accept/rollback.
5. Checkpoint E: HTTP 202 start endpoint, status/progression read model, frontend progression UI, full backend/frontend regression gates.
