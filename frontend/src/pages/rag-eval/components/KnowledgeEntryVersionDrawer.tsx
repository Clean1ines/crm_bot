...
export const KnowledgeEntryVersionDrawer: React.FC<{
  ...
  <div className="fixed inset-0 z-50 bg-black/40 p-4">
    <div className="ml-auto flex h-full max-w-3xl flex-col rounded-2xl bg-[var(--surface-elevated)] p-5 shadow-xl">
      <div className="flex items-center justify-between"><h2 className="text-lg font-semibold text-[var(--text-primary)]">{t('ragEval.curation.versions.title')}</h2><button type="button" onClick={onClose} className="text-sm text-[var(--text-muted)]">{t('common.actions.close')}</button></div>
      <div className="mt-4 space-y-3 overflow-auto">
        {!versions.length && <div className="rounded-xl bg-[var(--control-bg)] p-4 text-sm text-[var(--text-muted)]">{t('ragEval.curation.versions.empty')}</div>}
