...
export const KnowledgeCurationOverviewCard: React.FC<{ summary: KnowledgeCurationSummary }> = ({ summary }) => {
  const metrics = [
    [t('ragEval.curation.overview.totalEntries'), summary.total_entries],
    ...
    [t('ragEval.curation.overview.withoutSourceRefs'), summary.entries_without_source_refs],
    [t('ragEval.curation.overview.missingRetrievalRow'), summary.entries_missing_retrieval_surface],
  ...
  return (
    <section className="rounded-2xl bg-[var(--surface-elevated)] p-5 shadow-[var(--shadow-card)]">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold text-[var(--text-primary)]">{t('ragEval.curation.overview.title')}</h2>
        ...
        <div className={`inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm ${summary.document_processing_active ? 'bg-amber-500/10 text-amber-600' : 'bg-emerald-500/10 text-emerald-600'}`}>
          ...
          {summary.document_processing_active ? t('ragEval.curation.overview.processing') : t('ragEval.curation.overview.readyForReview')}
        </div>
      </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2 text-xs text-[var(--text-muted)]">
        <span className="inline-flex items-center gap-1"><GitMerge className="h-3.5 w-3.5" /> {t('ragEval.curation.overview.mergeNote')}</span>
        <span className="inline-flex items-center gap-1"><SearchX className="h-3.5 w-3.5" /> {t('ragEval.curation.overview.retrievalNote')}</span>
      </div>
    </section>
  );
};
