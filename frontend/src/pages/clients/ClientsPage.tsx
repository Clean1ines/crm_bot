...
export const ClientsPage: React.FC = () => {
  ...
  if (isLoading) {
    return <div className="flex justify-center p-4 text-sm text-[var(--text-muted)] sm:p-6 lg:p-8">{t('clients.loading')}</div>;
  }

  if (isError) {
    return (
      <div className="p-4 text-center text-sm text-[var(--text-muted)] sm:p-6 lg:p-8">
        {t('clients.loadFailed')}: {getErrorMessage(error)}
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-4 sm:p-6 lg:p-8 animate-in fade-in duration-500">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <h1 className="mb-2 text-2xl font-semibold leading-tight text-[var(--text-primary)] sm:text-3xl">{t('clients.title')}</h1>
          <p className="text-[var(--text-muted)]">{t('clients.description')}</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 sm:gap-4 lg:gap-6">
        <div className="bg-[var(--surface-secondary)] p-4 sm:p-5 lg:p-6 rounded-2xl">
          <div className="text-sm text-[var(--text-muted)] mb-1">{t('clients.stats.total')}</div>
          <div className="text-2xl font-semibold leading-none text-[var(--text-primary)]">{stats.total_clients}</div>
        </div>
        <div className="bg-[var(--surface-secondary)] p-4 sm:p-5 lg:p-6 rounded-2xl">
          <div className="text-sm text-[var(--text-muted)] mb-1">{t('clients.stats.new7d')}</div>
          <div className="text-2xl font-semibold leading-none text-[var(--accent-primary)]">+{stats.new_clients_7d}</div>
        </div>
        <div className="bg-[var(--surface-secondary)] p-4 sm:p-5 lg:p-6 rounded-2xl">
          <div className="text-sm text-[var(--text-muted)] mb-1">{t('clients.stats.activeDialogs')}</div>
        ...
        <div className="hidden overflow-x-auto md:block">
          <table className="w-full text-left border-collapse">
            <thead className="bg-[var(--surface-secondary)] shadow-[0_1px_0_var(--divider-soft)]">
              <tr>
                <th className="px-4 py-3 text-[11px] font-semibold uppercase tracking-wide text-[var(--text-muted)] lg:px-5">{t('clients.table.client')}</th>
                <th className="px-4 py-3 text-[11px] font-semibold uppercase tracking-wide text-[var(--text-muted)] lg:px-5">{t('clients.table.registrationDate')}</th>
                <th className="px-4 py-3 text-[11px] font-semibold uppercase tracking-wide text-[var(--text-muted)] lg:px-5">{t('clients.table.lastActivity')}</th>
                <th className="px-4 py-3 text-right text-[11px] font-semibold uppercase tracking-wide text-[var(--text-muted)] lg:px-5">{t('clients.table.actions')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[var(--surface-secondary)]">
              {clients.length === 0 ? (
                ...
                clients.map((client) => {
                  const displayName = getClientDisplayName(client, t('ui.client.fallback'));
                  const secondaryText = getClientSecondaryText(client) || t('clients.dialogsCount', { count: client.threads_count ?? 0 });
                ...
                >
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
                    <div className="rounded-xl bg-[var(--surface-card)] p-3">
                      <div className="text-[var(--text-muted)]">{t('clients.card.registration')}</div>
                      <div className="mt-1 font-semibold text-[var(--text-primary)]">
                        {createdAt ? createdAt.toLocaleDateString() : '—'}
                      </div>
                    </div>
                    <div className="rounded-xl bg-[var(--surface-card)] p-3">
                      <div className="text-[var(--text-muted)]">{t('clients.card.activity')}</div>
                      <div className="mt-1 font-semibold text-[var(--text-primary)]">
                        {lastActivityAt ? lastActivityAt.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '—'}
                      </div>
