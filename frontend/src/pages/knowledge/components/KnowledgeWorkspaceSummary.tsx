...
export const KnowledgeWorkspaceSummary: React.FC<{
  ...
  <section className="mb-6 rounded-2xl bg-[var(--surface-elevated)] p-4 sm:p-5">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div>
        <h2 className="text-base font-semibold text-[var(--text-primary)] sm:text-lg">{t(summary.headlineKey)}</h2>
        <p className="mt-1 text-sm text-[var(--text-muted)]">{t(summary.descriptionKey)}</p>
      </div>
      <button
        type="button"
      ...
      >
        {t(summary.primaryAction.labelKey)}
      </button>
    </div>
    ...
    <div className="mt-3 flex flex-wrap gap-1.5 text-[10px]">
      {summary.counters.slice(0, 4).map((counter) => (
        <span key={counter.id} className={`rounded-full px-2 py-0.5 ${toneClassName(counter.tone)}`}>
          {t(counter.labelKey)}: {counter.value}
        </span>
      ))}
    </div>
  </section>
